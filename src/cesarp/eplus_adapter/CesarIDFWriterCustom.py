...
from cesarp.eplus_adapter import idf_writer_geometry, idf_writer_geometry_custom
...
def copy_objects(objects, target_idf):
    for obj in objects:
...
class CesarIDFWriterCustom:
    ...
    def write_bldg_model(self, bldg_model: BuildingModel) -> None:
        ...
        copy_objects(zones, new_IDF)
        copy_objects(building_surfaces, new_IDF)
        copy_objects(fenestration_surfaces, new_IDF)

        new_IDF.save(filename=str(self.idf_file_path))
    ...
    def add_building_geometry(self, idf, bldg_shape_detailed: BldgShapeDetailed, constr_handler: ConstructionIDFWritingHandler):
        ...
        idf_writer_geometry_custom.add_basic_geometry_settings(idf)
        self.zone_data = idf_writer_geometry_custom.add_building(idf, bldg_shape_detailed, constr_handler)
