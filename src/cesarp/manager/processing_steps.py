...
def bldg_model_to_idf_no_exception(bldg_model, idf_file_path, profiles_files_handler, custom_config):
    ...
    try:
        ...
        temp_directory, temp_idf = os.path.split(idf_file_path)
        new_directory = os.path.join(temp_directory, "truncated_idfs")
        new_idf_file_path = os.path.join(new_directory, temp_idf)
        my_idf_writer_custom = CesarIDFWriterCustom(new_idf_file_path, unit_reg, profiles_files_handler,
